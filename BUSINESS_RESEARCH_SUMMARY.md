# BUSINESS RESEARCH — EXECUTIVE SUMMARY
## For 30-Minute Methodology Validation Meeting

**Document:** Reactacat Business Research  
**Date:** March 21, 2026  
**Purpose:** Present research methodologies, key findings, and justifications for validation

---

## 1. RESEARCH METHODOLOGY OVERVIEW

### Research Type
**Explanatory & Evaluative** — testing 5 business hypotheses using secondary quantitative data

### Methods Used
| Method | Application | Justification |
|--------|-------------|---------------|
| **Secondary Market Analysis** | Market sizing, growth rates (12–15.5% CAGR) | Primary surveys resource-intensive; industry reports sufficient for opportunity validation |
| **Competitive Analysis** | Feature/pricing gaps, positioning white space | Direct competitor websites, published reviews, public pricing |
| **Regulatory Documentation Review** | CE/UKCA pathways, laser safety (EN 60825-1), GDPR | Standards publicly available; compliance costs from practitioner sources |
| **Cost Benchmarking** | BOM estimates, cloud infrastructure (AWS) | Component pricing from suppliers, published AWS pricing |
| **Triangulation** | Cross-reference 3+ sources for critical claims | Reduces single-source bias; increases confidence |

### Data Sources (Secondary Only)
- **Market Reports:** Global Market Insights, Research Nester, Market Data Forecast, Future Market Insights (6 firms, consensus 12–15.5% CAGR)
- **Competitor Intelligence:** Petcube, Furbo, ZUMIMALL, Enabot (direct website analysis + tech media reviews)
- **Regulatory:** EN 60825-1, EU CE Directives, UKCA guidance, GDPR/UK GDPR frameworks
- **Veterinary:** Published sources on laser play frustration (Catster, Animal Wellness Magazine, Buffington 2002)

### What Was NOT Done (Acknowledged Limitations)
- No primary consumer surveys (deferred to user testing phase)
- No prospective interviews (secondary to market opportunity validation)
- No systematic review protocol (PRISMA) — narrative review appropriate for MBA scope

---

## 2. KEY FINDINGS SUMMARY

### H1: Market Growth
**Evidence found:**
| Metric | Finding | Source Confidence |
|--------|---------|-------------------|
| European pet tech market | €390–670M by 2026 | 6 research firms, 12–15.5% CAGR consensus |
| Cat ownership (EU + UK) | 89–108 million cats | FEDIAF 2024 |
| Per-capita pet spending | €300–400 annually | Market Data Forecast 2025 |
| Premium households target | 1.8–2.3M (€60K+ income) | Derived from FEDIAF + income data |

**Limitation:** "Autonomous cat play" subcategory not directly measured — derived estimate (€18–84M SAM)

---

### H2: Underserved Segment
**Finding:** No competitor combines: autonomous laser + adaptive AI + treat dispenser

| Competitor | Gap |
|------------|-----|
| Petcube Play 2 | Camera-first; laser is secondary; no AI adaptation |
| Furbo 360 | Dog-centric; no laser |
| ZUMIMALL | Random patterns; no learning |
| Enabot/Loona | General companion; not play-focused |

**Critical Uncertainty:** Treat dispenser mitigation for laser frustration — theoretically sound, requires cat testing validation

---

### H3: Subscription Viability
**Unit Economics (Base Case):**

| Metric | Year 1 | Year 3 | Notes |
|--------|--------|--------|-------|
| Hardware COGS | €99.50 | €75 | Custom PCB transition |
| Hardware margin | €41.50 (27.7%) | €66.50 (44.3%) | Improving with scale |
| Subscription conversion | 50% trial-to-paid | 60% | Critical assumption; SaaS benchmark 15–25% |
| LTV:CAC (per buyer) | 1.4:1 | ~2.5:1 | Marginal at launch; sustainable at scale |
| LTV:CAC (per subscriber) | 4.4:1 | 5.0:1 | Strong when conversion achieved |

**Assessment:** Year 1 economics marginal; sustainability depends on achieving 50% conversion + COGS reduction. Path to viability credible but carries execution risk.

---

### H4: Regulatory Feasibility
| Requirement | Cost | Timeline | Risk |
|-------------|------|----------|------|
| Laser safety (EN 60825-1) | €8–10K | 8–12 weeks | Low |
| CE marking (EU) | €3–5K | 4–8 weeks | Low |
| UKCA marking (UK) | €2–4K | 4–6 weeks (parallel) | Low |
| GDPR/UK GDPR compliance | €5–10K | Ongoing | Medium |
| **Total** | **€25–35K** | **6–12 months** | **Low-Medium** |

No regulatory showstoppers identified.

---

### H5: Competitive Differentiation
**Finding:** No existing product offers the specific combination Reactacat targets.

**Caveat:** Differentiation is real but market value unproven until user testing.

---

## 3. CRITICAL ASSUMPTIONS REQUIRING VALIDATION

| Assumption | Current Basis | Validation Plan |
|------------|---------------|-----------------|
| **50% trial-to-paid conversion** | Planning assumption (SaaS benchmark: 15–25%) | Poland launch metrics (Month 12 gate: 40% floor) |
| **Cat engagement with adaptive AI** | Theoretical advantage | Alpha testing with 10–20 cats, 4+ weeks |
| **Treat dispenser mitigates frustration** | Theoretically sound | Veterinary oversight during testing |
| **€45 CAC in European pet tech** | Benchmark estimate | Actual Poland campaign data |

---

## 4. RESEARCH QUALITY & LIMITATIONS

### Strengths
- Triangulation across 6 market research firms for growth rates
- Direct competitor analysis from primary sources (websites, reviews)
- Conservative financial modelling with sensitivity scenarios
- Explicit acknowledgment of limitations

### Limitations (Acknowledged)
| Limitation | Impact | Mitigation |
|------------|--------|------------|
| Proprietary market research methodologies | Uncertainty in TAM precision | Cross-firm consensus increases confidence |
| No primary consumer data | Pricing/conversion assumptions untested | Poland launch as validation phase |
| No controlled cat behaviour study | AI advantage unproven | User testing phase addresses this |
| Petcube financials not public | Competitive benchmarking incomplete | Inference from funding + market position |

### Methodological Rigor
- **Fit-for-purpose:** Secondary analysis appropriate for MBA capstone scope
- **Not academic systematic review:** Narrative review with transparent source identification
- **Actionable:** Results directly inform go/no-go decision for prototype phase

---

## 5. SYNTHESIS: GO/NO-GO ASSESSMENT

**Overall Verdict:** PROCEED TO PROTOTYPE & USER TESTING

**Rationale:**
- Market opportunity validated (growing, affluent, receptive)
- Competitive white space genuine
- Regulatory pathway clear
- Unit economics have credible path to sustainability

**Conditions for Continued Investment:**
1. Cat testing shows measurable engagement improvement vs. random play
2. Poland launch achieves ≥40% subscription conversion (Month 12 gate)
3. CAC remains ≤€55 in actual campaigns

**Contingency if Validation Fails:**
- <40% conversion → Pivot to hardware-only, reduced pricing
- <10% AI engagement improvement → Emphasize convenience over intelligence
- High CAC → B2B veterinary/shelter channel pivot

---

## 6. QUESTIONS FOR REVIEWER

1. **Methodology:** Is secondary quantitative analysis sufficient for MBA capstone, or should primary research have been prioritized?

2. **TAM Derivation:** Is our top-down SAM estimate (€18–84M) methodologically sound given lack of direct "autonomous cat play" category data?

3. **Triangulation:** Are 6 market research firms sufficient for confidence, or should we have sought additional independent sources?

4. **Conversion Assumption:** How should we justify 50% trial-to-paid given SaaS benchmarks of 15–25%? Is treat dispenser + AI improvement sufficient differentiation?

5. **Risk Acknowledgment:** Have we adequately disclosed limitations, or are there additional methodological concerns?

---

**Document Status:** Ready for methodology validation review  
**Next Step:** Incorporate reviewer feedback; proceed to user testing phase
