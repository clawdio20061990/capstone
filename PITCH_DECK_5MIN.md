# Reactacat 5-Minute Pitch Deck Structure

**Duration:** 5 minutes  
**Total Slides:** 10 (including visual placeholders)  
**Format:** Investor pitch / startup pitching session

---

## Slide 1: Title / Cover [VISUAL PLACEHOLDER]

**Visual:** Reactacat logo, clean minimal design, tagline

**Text:**
```
REACTACAT

AI-Powered Autonomous Cat Play System

[Your Name]
March 2026
```

**Speaker Notes:** [Silent slide, 3 seconds] → Immediately transition to Slide 2

---

## Slide 2: The Problem

**Visual:** Split image — lonely cat at home (left) + guilty owner at work (right)

**Text:**
```
50 Million Indoor Cats Are Bored

• 50% show signs of boredom/depression
• Owners work 8-10 hours daily, feel guilty
• Existing toys: boring, repetitive, cats lose interest

Cats need stimulation. Owners lack time.
```

**Speaker Notes (30 seconds):** "Fifty million cats across Europe spend most of their day alone. Studies show half of them exhibit signs of boredom or depression. Their owners — busy professionals — feel guilty but simply don't have time to play. Existing solutions don't work: random laser toys bore cats in days, manual toys require you to be home. This is a real, growing problem."

---

## Slide 3: Section Header — The Solution [VISUAL PLACEHOLDER]

**Visual:** Simple text on gradient background

**Text:**
```
Meet Reactacat
```

**Speaker Notes:** [2 seconds] → Quick transition

---

## Slide 4: Introducing Reactacat

**Visual:** Product hero shot, device in living room, cat playing

**Text:**
```
The First Truly Autonomous AI Cat Toy

✓ Plays automatically when you're away
✓ AI learns your cat's unique play style
✓ Gets smarter every day
✓ Completes hunt with treats

Not a camera. Not a remote toy.
A dedicated enrichment system.
```

**Speaker Notes (40 seconds):** "Reactacat is the first autonomous AI-powered cat play device. It doesn't need an app, doesn't need you to control it. Computer vision detects your cat, AI analyzes their behavior in real-time, and the laser adapts to what keeps them engaged. Every play session sends data to the cloud. Every night, the AI model retrains. Within weeks, Reactacat knows your cat better than any toy ever could. And it completes the hunting sequence with treats — addressing the frustration cats feel with uncatchable lasers."

---

## Slide 5: Market & Competition

**Visual:** EU map with market size + competitive positioning matrix

**Text:**
```
€50-70M EU Market | We Own the White Space

1.8-2.3M premium cat households
Pet tech growing 12-20% CAGR

Competitors:
• Petcube: Manual control, camera-first (€90-220)
• Cheap toys: Random, boring (€10-30)
• Us: Autonomous + Intelligent (€150)

No one else does both.
```

**Speaker Notes (35 seconds):** "The European pet tech market is booming. We're targeting 1.8 to 2.3 million premium households — people who see cats as family and already spend €600+ per year. Look at the competition: Petcube is a camera that requires manual control. Cheap toys are random and boring. We're the only product that's both autonomous and genuinely intelligent. This is a white space opportunity."

---

## Slide 6: Section Header — The Business [VISUAL PLACEHOLDER]

**Visual:** Simple icon (€ symbol or revenue chart icon)

**Text:**
```
Business Model & Traction
```

**Speaker Notes:** [2 seconds] → Quick transition

---

## Slide 7: Business Model & Unit Economics

**Visual:** Revenue model diagram, LTV:CAC ratio graphic

**Text:**
```
Hardware + Subscription Model

€150 hardware (one-time)
€3-8/month subscription (3 tiers)

LTV: €270
CAC: €45
LTV:CAC = 6:1

Target: 50% subscription conversion
Break-even: Month 28-30
```

**Speaker Notes (35 seconds):** "We're not just selling hardware. €150 gets you the device. But the ongoing value is in the subscription — €3 to €8 per month depending on tier. The AI only keeps learning with a subscription. We're targeting 50% conversion. At that rate, lifetime value is €270, acquisition cost is €45. That's 6-to-1 LTV to CAC. Healthy unit economics from day one. Break-even at Month 28 to 30."

---

## Slide 8: Traction & Validation

**Visual:** Survey results bar charts, prototype testing photo

**Text:**
```
Market Validation: Cats Love It

Survey (100+ owners):
• 73% want autonomous play device
• 58% will pay €120-150

Prototype Testing (15 households):
• 87% of cats engaged
• 60% increased activity
• 80% would recommend

"My cat runs to it now. Never seen him this excited."
— Warsaw beta tester
```

**Speaker Notes (30 seconds):** "We've validated demand. Over 100 cat owners surveyed — 73% want this, 58% will pay our price. We tested prototypes with 15 real households. 87% of cats engaged. 80% of owners would recommend it. This isn't theory. We've proven product-market fit."

---

## Slide 9: Growth Plan & Financials

**Visual:** 3-year revenue chart, expansion map (Poland → EU)

**Text:**
```
Path to €2.88M Revenue, Profitability Year 3

Year 1: Poland launch → 1,200 units | €188K revenue
Year 2: EU expansion → 6,500 units | €1.06M revenue
Year 3: Scale → 18,000 units | €2.88M revenue | +€115K profit

Poland → Germany/France/UK → Full EU
Gross margin: 22% → 59% (custom PCB transition)
```

**Speaker Notes (35 seconds):** "Year 1 is Poland — our home market. 1,200 units, validate unit economics. Year 2, we expand to Germany, France, UK. By Year 3, we're at 18,000 units and profitable. Revenue grows from €188K to €2.88M. Gross margins improve dramatically — 22% to 59% — as we transition from Raspberry Pi to custom hardware. Clear path to scale."

---

## Slide 10: The Ask

**Visual:** Funding allocation pie chart, contact information

**Text:**
```
€650K Seed Round
18-Month Runway to Series A

Use of Funds:
• Product Development: 28%
• Marketing & Sales: 28%
• Inventory & Operations: 23%
• Team (2 FTE): 18%
• Legal & Regulatory: 3%

Milestones:
✓ Poland launch (Month 7)
✓ 1,000 units sold (Month 12)
✓ Validated CAC & conversion (Month 15)

Let's talk.
[Your Email] | [Phone]

Questions?
```

**Speaker Notes (35 seconds):** "We're raising €650,000. This gives us 18 months to launch in Poland, sell 1,000 units, and prove our unit economics. By Month 18, we'll be positioned for Series A to fuel EU expansion. The allocation is clear: product, marketing, operations, team. If you believe in this opportunity, let's talk. [Pause for questions]"

---

## Backup Slides (if time permits or for Q&A)

### B1: Team
- Your background
- Co-founder (if applicable)
- Key advisors

### B2: Key Risks & Mitigation
- Laser safety → addressed
- Supply chain → buffers + custom PCB
- Competition → speed + data moat

### B3: Vision Beyond Year 3
- Product line expansion
- B2B opportunities
- Platform strategy

---

**Document Status: COMPLETE**  
**Slide Count:** 10 main slides (3 are visual placeholders/section headers)  
**Speaking Slides:** 7 content slides  
**Total Speaking Time:** ~4:30 (leaves 30s buffer)  
**Backup Slides:** 3 (for Q&A)
